    ...
    listenIps: [
      {
        ip: '0.0.0.0',
        announcedIp:
          process.env.NODE_ENV === 'development' ? '127.0.0.1' : '51.250.35.29',
      },
    ],
    initialAvailableOutgoingBitrate: 600000,
    minimumAvailableOutgoingBitrate: 500000,
    maximumAvailableOutgoingBitrate: 3500000,
    factorIncomingBitrate: 1.5,
  },
};
...
export class WssRoom {
        ...
        .then(() => {
          ...
          this.audioLevelObserver.on(
            ...
            (volumes: Array<{ producer: Producer; volume: number }>) => {
              this.wssServer.to(this.session_id).emit('mediaActiveSpeakers', {
                activeSpeakers: volumes.reduce(
                  (acc, key) => {
                    acc[key.producer.appData.user_id as string] = true;
                    return acc;
                  },
   ...
   * @param {Socket} client клиент
   * @param {string} event ивент из сообщения
   * @param {msg} msg сообщение клиента
   * @returns {boolean} boolean
   */
  public broadcast(client: Socket, event: string, msg: object): boolean {
    try {
  ...
  public async speakMsClient(
    ...
    try {
      switch (msg.action) {
        ...
        case 'pauseAudioConsumer':
          return await this.pauseAudioConsumer(msg.data as { user_id: string });
        case 'resumeAudioConsumer':
          return await this.resumeAudioConsumer(
            msg.data as { user_id: string },
          );
  ...
  private async consume(
    ...
    try {
      ...
      const userInfo = await this.prisma.serverMembership.findFirst({
        where: {
          user: { id: parseInt(user_id) },
          channel: { isNot: null },
        },
        select: { isServerDeafen: true, user: { select: { isDeafen: true } } },
      });
      ...
      switch (data.kind) {
        ...
        case 'audio':
          ...
          if (userInfo.isServerDeafen || userInfo.user.isDeafen) {
  ...
  /**
   * Приостанавливает получения аудиостримов юзера
   * @param {string} _user_id автор сообщения
   * @returns {Promise<boolean>} Promise<boolean>
   */
  private async pauseAudioConsumer({
    user_id,
  }: {
    user_id: string;
  }): Promise<boolean> {
    try {
      ...
      client?.media?.consumersAudio?.forEach((consumer) => {
        if (!consumer.closed) {
          consumer.pause();
        }
      });
  ...
  /**
   * Возобновляет получения аудиостримов юзера
   * @param {string} _user_id автор сообщения
   * @returns {Promise<boolean>} Promise<boolean>
   */
  private async resumeAudioConsumer({
    user_id,
  }: {
    user_id: string;
  }): Promise<boolean> {
